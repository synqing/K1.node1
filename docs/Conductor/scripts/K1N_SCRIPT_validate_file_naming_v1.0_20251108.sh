...
for FILE in $STAGED_FILES; do
    ...
    # Skip validation for files outside docs/Conductor directory
    if [[ ! "$DIR" =~ ^docs/Conductor ]]; then
        echo "✓ $FILE (outside docs/Conductor, skipped)"
        continue
    fi
    
    # Skip validation for files in scripts/ directory (executables)
    if [[ "$DIR" =~ ^docs/Conductor/scripts ]]; then
...
echo "=========================================="
if [ $ERRORS -eq 0 ]; then
    echo "✅ All docs/Conductor files comply with naming standard"
    echo "=========================================="
    echo ""
    exit 0
else
    ...
    echo "=========================================="
    echo ""
    echo "Fix filenames in docs/Conductor/ to match: [ProjectCode]_[Type]_v[Version]_[YYYYMMDD].[ext]"
    echo ""
    exit 1
fi
