...
/**
 * Pattern: PRISM - Hero Demo Pattern
 *
 * A spectacular frequency visualization with beat-synchronized energy response.
 * Combines spectrum bars with dynamic color mapping and smooth animations.
 *
 * Design Goals:
 * - Immediate response to music (frequency + energy)
 * - Visually impressive (vibrant colors, smooth motion)
 * - Works great with or without audio (idle pulsing)
 * - Center-origin architecture (symmetric, balanced)
 *
 * Architecture:
 * - Core: 64 frequency bins mapped to 160 LEDs with center-origin mirroring
 * - Energy: Beat-synchronized brightness boost on peak detection
 * - Color: Hue from frequency (blue→red), saturation from energy level
 * - Animation: Frame persistence with exponential decay for smooth trails
 * - Idle: Soft pulsing sine wave when silent
 */
void draw_prism(float time, const PatternParameters& params) {
    ...
    // Audio age decay for smooth silence transition
    float age_ms = (float)AUDIO_AGE_MS();
    float age_factor = 1.0f - fminf(age_ms, 500.0f) / 500.0f;
    age_factor = fmaxf(0.0f, age_factor);

    // Trail decay speed based on 'softness' param (persistence effect)
    float trail_decay = 0.88f + 0.08f * clip_float(params.softness);

    // Apply decay to trail buffer
    for (int i = 0; i < NUM_LEDS; i++) {
        prism_trail[i] *= trail_decay;
    }

    // Energy gate: combine VU level + spectral novelty for beat detection
    float energy_level = 0.0f;
    if (AUDIO_IS_AVAILABLE()) {
        energy_level = fminf(1.0f, (AUDIO_VU * 0.8f) + (AUDIO_NOVELTY * 0.3f));
    }

    // Beat detection with threshold (custom_param_1 = beat sensitivity)
    ...
    // Energy boost on beats (glow effect)
    float energy_boost = 1.0f + beat_factor * 0.6f;

    // Render spectrum to center-origin
    int half_leds = NUM_LEDS / 2;

    for (int i = 0; i < half_leds; i++) {
        float progress = (float)i / (float)half_leds;

        // Get frequency magnitude with smooth interpolation
        float magnitude = 0.0f;
        if (AUDIO_IS_AVAILABLE()) {
            magnitude = clip_float(AUDIO_SPECTRUM_INTERP(progress));
            // Apply square root for better visual perception
            magnitude = response_sqrt(magnitude);
            // Boost on beats, fade on silence
            magnitude = magnitude * energy_boost * age_factor;
        } else {
            // Idle animation: soft pulsing sine wave
            magnitude = 0.5f + 0.3f * sinf(time * 2.0f + progress * 3.14159f);
        }

        // Clamp magnitude to valid range
        magnitude = fmaxf(0.0f, fminf(1.0f, magnitude));

        // Update trail buffer with current magnitude
        int half_center = half_leds - 1;
        prism_trail[half_center - i] = fmaxf(prism_trail[half_center - i], magnitude);
        prism_trail[half_leds + i] = fmaxf(prism_trail[half_leds + i], magnitude);

        // Color mapping: hue from frequency position, saturation from energy
        float hue = progress;  // Blue (0.0) → Red (1.0)
        float saturation = 0.85f + 0.15f * clip_float(energy_level);

        // Get color from HSV
        CRGBF color = hsv(hue, saturation, magnitude);

        // Apply global brightness parameter
        color.r *= params.brightness;
        color.g *= params.brightness;
        color.b *= params.brightness;

        // Mirror to both sides from center
    ...
    // Apply trail glow effect on top of spectrum
    float trail_strength = 0.2f * clip_float(params.custom_param_2);  // Adjustable trail intensity
    for (int i = 0; i < NUM_LEDS; i++) {
        if (prism_trail[i] > 0.05f) {
            // Blend trail color (tinted white/bright) with current LED color
...
const PatternInfo g_pattern_registry[] = {
	...
	// Domain 2: Audio-Reactive Patterns
	{
		"Prism",
		"prism",
		"★ DEMO ★ Frequency + beat-synchronized energy response with trails",
		draw_prism,
		true
	},
	{
