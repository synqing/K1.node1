...
export class CronParser {
   ...
   */
  nextExecution(from: Date = new Date()): Date | null {
   ...
   */
  previousExecution(from: Date = new Date()): Date | null {
    if (!this.isValid()) return null;
  ...
  /**
   * Parse a single cron field (e.g., "0", "*/5", "1-5", "1,3,5")
   */
   ...
   */
  private matchesSchedule(date: Date): boolean {
    ...
    const dayAndWeekdayMatch =
      (this.day.length === 31 && this.weekday.length === 7) ||
      this.day.length === 31 ||
 ...
 */
export function getPreviousExecutionTime(
  cronExpression: string,
  fromDate: Date = new Date()
): Date | null {
  const parser = new CronParser(cronExpression);
