...
export class ScheduleExecutor implements IScheduleExecutor {
  ...
  constructor(
    ...
    private webhookExecutor?: (workflowId: string) => Promise<void>
  ) {}
   ...
   */
  private async executeSchedule(scheduleId: string, workflowId: string): Promise<void> {
    ...
    try {
      ...
      console.error(
        `Failed to execute schedule ${scheduleId}: ${errorMessage} (duration: ${durationMs}ms)`
      );
    }
  }
}
 ...
 */
export function createScheduleExecutor(
  schedulerService: ISchedulerService,
  webhookExecutor?: (workflowId: string) => Promise<void>
): IScheduleExecutor {
