...
export class RetryEngine {
   ...
   */
  applyJitter(delayMs: number, jitterFactor: number = 0.1): number {
  ...
  ): RetryCalculationResult {
    let delayMs: number;

    switch (policy.strategy) {
      case 'exponential':
        delayMs = this.calculateExponentialBackoff(
          attempt,
        ...
        break;
      case 'linear':
        delayMs = this.calculateLinearBackoff(
          attempt,
          policy.initialDelayMs,
          policy.maxDelayMs
        );
        break;
      case 'fixed':
        delayMs = this.calculateFixedBackoff(
          policy.initialDelayMs,
          policy.maxDelayMs
        );
        break;
   ...
   */
  async getDueRetries(): Promise<RetryAttempt[]> {
