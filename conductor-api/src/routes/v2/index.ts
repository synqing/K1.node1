...
import { AuthenticatedRequest } from '../../middleware/v2-authentication';
import { authMiddlewareV2, requireScopesV2, requireAuthV2, SCOPES_V2 } from '../../middleware/v2-authentication';
...
export const createV2Router = (): Router => {
   ...
   */
  router.use(authMiddlewareV2 as any);
  ...
  router.use((req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    ...
    if (process.env.DEBUG_REQUESTS === 'true') {
      console.log(
        `[API v2] ${req.method} ${req.path} [${client}${user ? ':' + user : ''}] (${correlationId})`
      );
    }
