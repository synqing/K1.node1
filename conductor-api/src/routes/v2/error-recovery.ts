...
import type { RetryPolicy, RetryAttempt } from '../../types/retry-policy.types.js';
import type {
  CircuitBreakerState,
  CircuitBreakerMetrics,
} from '../../types/circuit-breaker.types.js';
import type { DLQEntry, DLQStats } from '../../types/dlq.types.js';
...
export class ErrorRecoveryController {
   ...
   */
  async createRetry(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const retryAttempt = await this.retryEngine.createRetryAttempt(
        taskId,
        attemptNumber,
        errorMessage,
        retryPolicy
   ...
   */
  async getRetry(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { id } = req.params;
   ...
   */
  async updateCircuitBreaker(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      let result: CircuitBreakerState | null = null;
   ...
   */
  private isValidRetryPolicy(policy: RetryPolicy): boolean {
 ...
 */
export function createErrorRecoveryRouter(
  retryDb: RetryDatabase,
  circuitBreakerStorage: CircuitBreakerStorage,
  dlqStorage: DLQStorage,
  circuitBreakerConfig: {
  ...
  return {
    ...
    'POST /v2/errors/retry': (req: Request, res: Response, next: NextFunction) =>
      controller.createRetry(req, res, next),

    // GET /v2/errors/retry/:id - Get retry details
    'GET /v2/errors/retry/:id': (req: Request, res: Response, next: NextFunction) =>
      controller.getRetry(req, res, next),

    // POST /v2/errors/resolve - Mark error resolved
    'POST /v2/errors/resolve': (req: Request, res: Response, next: NextFunction) =>
      controller.resolveError(req, res, next),

    // GET /v2/errors/stats - Get statistics
    'GET /v2/errors/stats': (req: Request, res: Response, next: NextFunction) =>
      controller.getStats(req, res, next),

    // POST /v2/errors/circuit-breaker/:service - Update circuit breaker
    'POST /v2/errors/circuit-breaker/:service': (req: Request, res: Response, next: NextFunction) =>
      controller.updateCircuitBreaker(req, res, next),
  };
}
