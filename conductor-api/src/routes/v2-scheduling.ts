...
export function createSchedulingRouter(
  ...
  router.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      const { name, cronExpression, workflowId, enabled, description, timezone, metadata } =
        req.body;
      ...
      if (!cronValidation.valid) {
        throw new ValidationError(
          `Invalid cron expression: ${cronValidation.error || 'Unknown error'}`
        );
      }
  ...
  router.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      try {
        ...
        const execution = await schedulerService.recordExecution(
          id,
          schedule.workflowId,
        ...
        await schedulerService.recordExecution(
          id,
          schedule.workflowId,
  ...
  router.get(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      // Build filter options
      const filterOptions: ExecutionHistoryFilterOptions = {
        limit: pageSize,
        offset,
        scheduleId: id,
      };
      ...
      if (status) {
        ...
        if (!validStatuses.includes(status as ScheduleStatus)) {
          throw new ValidationError(
            `Invalid status: ${status}. Valid statuses: ${validStatuses.join(', ')}`
          );
        }
        filterOptions.status = status as ScheduleStatus;
      }
