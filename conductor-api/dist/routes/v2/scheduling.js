 ...
 */
const validateScheduleRequest = (body) => {
    const errors = [];
    if (!body.task_id)
 ...
 */
router.post('/', requireAuthV2, requireScopesV2(SCOPES_V2.SCHEDULER_WRITE), async (req, res) => {
    try {
        const { task_id, cron_expression, description, enabled = true, max_concurrent_runs = 1, timezone = 'UTC', } = req.body;
        ...
        res.status(201).json(formatResponse('success', schedule, undefined, correlationId));
    }
    catch (error) {
        const correlationId = req.correlationId;
 ...
 */
router.get('/:id', requireScopesV2(SCOPES_V2.SCHEDULER_READ), async (req, res) => {
    try {
        const { id } = req.params;
        const correlationId = req.correlationId;
        // Validate ID format
 ...
 */
router.put('/:id', requireAuthV2, requireScopesV2(SCOPES_V2.SCHEDULER_WRITE), async (req, res) => {
    try {
        const { id } = req.params;
 ...
 */
router.get('/', requireScopesV2(SCOPES_V2.SCHEDULER_READ), async (req, res) => {
    try {
 ...
 * Delete a schedule
 */
router.delete('/:id', requireAuthV2, requireScopesV2(SCOPES_V2.SCHEDULER_WRITE), async (req, res) => {
    try {
        const { id } = req.params;
        const correlationId = req.correlationId;
        // Validate ID format
