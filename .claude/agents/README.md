...
You can install additional agents and tools from the community marketplace. One high-quality source is the `wshobson/agents` marketplace.

Usage:
- Add marketplace: `/plugin marketplace add wshobson/agents`
- List and install: `/plugin`, then `/plugin install <plugin-name>`
- Examples:
...
Notes:
- Each plugin loads only its own agents, commands, and skills (progressive disclosure).
- Keep installations minimal and targeted to your workflow.
